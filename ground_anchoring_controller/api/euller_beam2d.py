...
class clWall():
    ...
    class clBeamAnalytical():
        ...
        def xy_get(self,v_x_anchor,nPoints=300,bGraph=True):
            ...
            if bGraph:
                ...
                #print(str(MyMath.graphDir))
                #      graphDir(self,vx,f=None,vy=None,kMax=4,dx=None,bMoreMax=False,sf="f")
                #print("vvvvv"+str(vx))
                #vx=[0,1,2,3]
    ...
    def testBeam(self):
        #self.h=10
        self.yMax=10
        self.angleFromVerticalGrad=30
        self.start()
        
        
        anchors=self.vx_anchors_get(0)
        MyMath=clMyMath()
        ...
        if bA:
            ...
            x_data,y_data=BeamAnalytical.xy_get(anchors,100)
            MyMath.testSol(x_data,y_data,kMin=0,kMax=4,sf="w",sTitle="Analytic solution: for selected points only (no w''''in anchors and in the end)",vxNoNeedHighestDiff=anchors)
        ...
        if bN:
            ...
            BeamNumerical.MomentMax_get()
            MyMath.testSol(x_data,y_data,kMin=0,kMax=4,sf="w",sTitle="Numerical solution: for selected points only (no w''''in anchors and in the end)",vxNoNeedHighestDiff=anchors)
        return
...
class clMyMath():
    ...
    def testSol(self,x_data,y_data,kMin=0,kMax=4,sf="f",sTitle="sTitle",vxNoNeedHighestDiff=[]):#x1_data,y1_data
        ...
        #def ff(x):
        #	return x**10
        #MyMath.graphDir(vx,f=ff,vy=vy,kMax=kMax,dx=None,bMoreMax=False,sf=sf)
